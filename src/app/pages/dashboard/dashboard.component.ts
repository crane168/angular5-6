...
export enum fileOperatorType {moveFile = 1, sendMail = 2, downloadFile = 3};
...
export class DashboardComponent extends PageComponent {
    ...
    protected onPageInit() {
        ...
        if(this.ctx.userId!==1){
            // 初始化filelist页面
            this.searchService.renderSearchResult(this.searchService.request).subscribe(res => {
    ...
    protected  downUriBy(item:any):any{
        return this.ctx.fileHost()+'&fileId='+item.id+'&fileName='+item.name;
    }

    // 更新上传文件
    protected updateUpload(fileInfo){
        // 验证是否有更新权限
        this.fileService.checkUploadPermission({id:fileInfo.dictId}).subscribe(res=>{
            if(res.xeach===true){
                this.router.navigate(["/portal/"+this.ctx.domain+"/uploadFile",fileInfo.dictId]);
            }else{
    ...
    setGrant(grantInfo){
        ...
        this.dlgAutherize.afterClosed().subscribe(result=>{
            ...
            self.fileService.grantUploadPermission({
                id: grantInfo.id,
    ...
    // 点击filter筛选事件
    protected renderFilterResult(e, conditionIndex, termIndex, conditionName, term) {
    ...
    doSort(e) {
        ...
        this.searchService.request['sort'] = {"name": "timestamp", "order": this.sortBy};
        this.searchService.renderSearch();
    }

    //切换更多
    toggleMoreCategories(changeId) {
        this.searchService.conditions[changeId].isShowMore = !this.searchService.conditions[changeId].isShowMore;
    }
        ...
        switch (type) {
            case 1://移动文件
                ...
                this.dlgFolder.afterClosed().subscribe(result => {
                    if (!result || (self.folderInfo && self.folderInfo.id == result.id && self.folderInfo.name == result.name)) {
                        this.operateFiles.length = 0;
                        return;
                    }
                    self.folderInfo = result;
                    this.fileService.pickup({
    ...
    showDetail(e, fileId,dictId) {
        // 跳转到详情页
        this.router.navigate(['/portal/' + this.ctx.domain + '/detail',fileId,dictId]);
        e.stopPropagation();
    }
