...
export class FileUploader {
    ...
    public totalSize(){
        ...
        for(var i=0;i<this.queue.length;i++){
            var  item:FileItem=this.queue[i];
            result+=item.size();
        }
        return result;
    }
    ...
    protected checkFileName(file:FileLikeObject){
    ...
    public addToQueue(files: File[], options?: FileUploaderOptions, filters?: FilterFunction[] | string): void {
        ...
        list.map((some: File) => {
            ...
            /*TODO:校验文件名是否与以前的文件名一致*/
            let temp = new FileLikeObject(some);
            let isValidFileName:boolean=this.checkFileName(temp);
            if (isValidFileName && this._isValidFile(temp, arrayOfFilters, options)) {
    ...
    protected _xhrTransport(item: FileItem): any {
        ...
        /*michael.xiao add*/
        let file=item.file;
        xhr.setRequestHeader("FileName", new Buffer(file.name).toString('base64'));
        xhr.setRequestHeader("FileType", file.type);
        xhr.setRequestHeader("FileSize", file.size.toString(10));
        xhr.setRequestHeader("FileDate", file.rawFile["lastModified"].toString(10));
